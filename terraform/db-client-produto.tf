...
resource "aws_instance" "db_client_produto" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "mysql -h ${aws_db_instance.mysql_rds_produto.address} -u ${var.mysql_user_produto} -p${var.mysql_password_produto} ${var.mysql_database_produto} < /home/ec2-user/init.sql" # Caminho do arquivo SQL na instância
    ]
